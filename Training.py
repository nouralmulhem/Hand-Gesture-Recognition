...
def failures(ytest, y_pred, name_test, true_class = 4, prediction_class = 0):
    ...
    listaia = new_arr[np.logical_and(pred == prediction_class, test == true_class)]
    print(listaia.reshape(-1, 1))
    
    listaia = np.concatenate((np.concatenate((new_arr, pred), axis = 1), test), axis = 1)
...
classifier(Xtrain, Xtest, ytrain, ytest, name_train, name_test, './models/svm2.pkl', show_fails = True)
